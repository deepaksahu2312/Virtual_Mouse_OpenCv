...
class handDetector():
    def __init__(self, mode=False, maxHands=2, detectionCon=0.5, trackCon=0.5):         #this is alll are iniitalization of object
        ...
        self.hands = self.mpHands.Hands(self.mode, self.maxHands, self.detectionCon, self.trackCon)         #object of hands class eith different parameters
    ...
    def findHands(self, img, draw=True):#for drawing hands only
        ...
        #we have this method provideed by mediapipe that actually helps us to draw all the points (total21points are there)
    ...
    def findPosition(self, img, handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:#check tha5 htere we are detecting any landmark or not
            ...
            for id, lm in enumerate(myHand.landmark):#this landmarke what we are getting and id is which is related to the exact id of landmark
                ...
                cx, cy = int(lm.x * w), int(lm.y * h)#position of the center in decimal
                xList.append(cx)
                yList.append(cy)
...
def main():
    ...
    while True:
        ...
        fps = 1 / (cTime - pTime)       #fps : frame rate ratio
